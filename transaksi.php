...
<?php
  if ($_SERVER["REQUEST_METHOD"] == "GET" && !empty($_GET["key"])) {
    ...
    $_pr = sha1($_SESSION["email"] . "I" . $_SESSION["trx-rpt-time"]);
    $_dl = sha1($_SESSION["email"] . "D" . $_SESSION["trx-rpt-time"]);
    if ($_key ==  $_pr || $_key == $_dl) {
      ...
      $title = 'transaksi-' . $_SESSION["email"] . '-' . $access['date'] . '-' . $access['time'] . '.pdf';
      ...
      generate_trx_access($access);
          ...
          function FancyTable($data) {
            ...
            if (count($data) == 0) {
              ...
              for ($g = 0; $g < count($data); $g++) {
                ...
                $trx_date = date_create($data[$g]["header_date"]);
                $current_data_1 = array(
                  $data[$g]["header_id"],
                  switch_payment($data[$g]["header_type"]) . "/" . $data[$g]["header_payment_methods"],
                  empty_to_strip($data[$g]["header_information"])
                );
                $current_data_2 = array(
                  switch_day(date_format($trx_date, "w")) . ", " . date_format($trx_date, "j") . " " . switch_month(date_format($trx_date, "m")) . " " . date_format($trx_date, "Y") . "/" . $data[$g]["header_time"],
                  $data[$g]["user_email"],
                  $data[$g]["admin_email"]
                );
                for ($h = 0;$h < count($header_main_2); $h++) {
                ...
                //$this->SetFont('DroidSerif','',9);
                //$this->Cell(100,5,$data[$g]["header_information"],0,0,'L',true);
                //$this->Ln(11);

                $details = get_user_trx_details('*', $data[$g]["header_id"]);
                if (count($details) == 0) {
        ...
        $data = get_user_trx_header($_SESSION["email"]);
        $pdf->FancyTable($data);
